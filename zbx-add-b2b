...
# Modelos (b2b.exemplo.*)
DEFAULT_MODEL_APP = "b2b.exemplo.app.host"
DEFAULT_MODEL_DB  = "b2b.exemplo.db.host"
# ==============================
...
def rpc(zbx_url, zbx_token, method, params, timeout=30):
    """
    Zabbix 7.2+: token vai no header Authorization: Bearer <token>.
...
def minimalize(list_of_dicts, key):
    ...
    for d in list_of_dicts or []:
        if key in d: out.append({key:d[key]})
    return out
...
def parse_kv_list(items, allow_empty_value=False):
    ...
    for item in items or []:
        if "=" not in item:
            if allow_empty_value: k,v=item,""
            else: die("Par inválido (esperado chave=valor): {}".format(item))
        else:
            k,v=item.split("=",1)
        parsed.append((k,v))
    return parsed
...
def build_interfaces_from_model(model, new_ip):
    ...
    for itf in interfaces:
        ...
        if t==1 and m==1:
            itf_new["useip"]=1; itf_new["ip"]=new_ip; itf_new["dns"]=""
            agent_main_set=True
...
def ensure_macro(macros, macro_name, value):
    ...
    for m in macros:
        if m.get("macro")==macro_name:
            m["value"]=value; found=True; break
    if not found:
...
def merge_groups_by_ids(existing_groups, extra_group_ids):
    ...
    for g in existing_groups or []:
        ...
        if gid and gid not in seen:
            base.append({"groupid":gid}); seen.add(gid)
...
def validate_ip(value, label):
    try:
        ipaddress.ip_address(value); return value
    except Exception:
        die("IP inválido para {}: {}".format(label, value))

def validate_https_url(url_str, label):
    if not url_str:
        die("{} é obrigatório.".format(label))
    parsed = urlparse(url_str.strip())
    if parsed.scheme.lower() != "https":
        die("{} deve usar HTTPS (ex: https://exemplo.com/path). Valor recebido: {}".format(label, url_str))
    if not parsed.netloc:
        die("{} inválida (sem host). Valor recebido: {}".format(label, url_str))
...
def clone_host(zbx_url, zbx_token, model_host, new_hostname, new_ip,
    ...
    for t in model.get("tags") or []:
        tag={"tag":t.get("tag","")}
        if "value" in t: tag["value"]=t.get("value","")
        tags.append(tag)
    ...
    for m in model.get("macros") or []:
        ...
        if "type" in m:
            try: entry["type"]=int(m.get("type",0))
    ...
    if proxy_hostid: params["proxy_hostid"]=str(proxy_hostid)
    if copy_inventory:
...
def parse_args():
    p = argparse.ArgumentParser(description="Criar hosts B2B (APP/DB) clonando de modelos b2b.exemplo.*.")
    ...
    p.add_argument("--url-admin", help="URL ADMIN da loja (HTTPS obrigatório) -> macro {$URL_TO_CHECK}")
...
def main():
    ...
    if not shop:
        try: shop = input("Informe o shop_name (ex: novaloja2): ").strip()
        except KeyboardInterrupt: die("\nCancelado.")
    if not app_ip:
        try: app_ip = input("IP do host APP: ").strip()
        except KeyboardInterrupt: die("\nCancelado.")
    if not db_ip:
        try: db_ip = input("IP do host DB: ").strip()
        except KeyboardInterrupt: die("\nCancelado.")
    if not url_admin:
        try: url_admin = input("Insira a URL ADMIN da loja (HTTPS obrigatório): ").strip()
        except KeyboardInterrupt: die("\nCancelado.")

    if not shop: die("shop_name é obrigatório.")
    validate_ip(app_ip, "APP")
    validate_ip(db_ip, "DB")
    url_admin = validate_https_url(url_admin, "URL ADMIN")

    new_app = "b2b.{}.app.host".format(shop)
    new_db  = "b2b.{}.db.host".format(shop)
